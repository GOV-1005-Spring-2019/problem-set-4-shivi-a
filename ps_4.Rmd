...
orig %>% 
  ...
  # Deselect the row total column and the 3rd party voter percentages as this data is not displayed in the final chart
  select(-all, -`3`) %>% 
  ...
  # Style certain cells to have a red background
      tab_style(style=cells_styles(
        bkgd_color = "crimson", 
        text_color = "white"), 
        locations = cells_data(columns = vars(Rep), rows = Rep > 0.31)) %>%
  
  # Style certain cells to have a blue background
  tab_style(style=cells_styles(
    bkgd_color = "blue", 
    text_color = "white"), 
    locations = cells_data(columns = vars(Dem), rows = Dem > 0.46)) %>%
...
# For my visualization, I was interested to look at the distribution of votes (broken down by response) over the different timestamps. 

# I noticed that the date range for the timestamps spanned 10-26 to 10-31. Even if I was to look at all the data together, I wouldn't neccessarily expect to see an even distribution of 1/6 of the voters coming in on each day - the result of various factors that make some days more convenient than others. 

# I wondered what the patterns might be for each response group - e.g. did the Third Party voters tend to all vote right at the end? Did Republican voters tend to vote earlier in the voting window, or later?
...
orig %>% 
  ...
  # Calculate what proportion of the total in a response group voted on a given day, and then convert this to a percentage
  ...
  # Create bar chart; the position_dodge function and argument ensures that bars are uniform size and that spaces are left for "missing" data - allowing us to visually understand that there were zero Third Party or Undecided voters on the last day
  ...
  labs(title = "North Carolina 9th - Voting Days Vote Distribution 2018", 
       subtitle = "The largest percent of Democrats voted on 10/27, while Republican turnout had a more even 
distribution, except the last day", 
